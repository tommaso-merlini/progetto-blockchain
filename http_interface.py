    ...
    async def post(url: str, payload: dict) -> dict:
        def _sync_post():
            ...
            try:
                ...
                raise RuntimeError(f"{url} ha risposto HTTP {e.code}: {detail}") from e
            except URLError as e:
                raise RuntimeError(f"Connessione fallita verso {url}: {e.reason}") from e
            except ValueError as e:
                raise RuntimeError(f"URL non valido: {url}. Usa un URL completo, per esempio http://127.0.0.1:1234") from e
