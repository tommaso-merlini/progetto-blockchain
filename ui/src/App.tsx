...
type ActiveDialog = "propose" | "accept" | null;
...
function App() {
  ...
  }, [channelPeerUrls, channels, lastPeerUrl]);
  ...
  async function refresh(
    ...
    try {
      const [nextPublicKey, nextStatus] = await Promise.all([
  ...
  function handleFund(event: FormEvent<HTMLFormElement>) {
    ...
    void runAction(
      () =>
        apiPost<{ funding_id: string }>(normalizedApiUrl, "/client/fund", {
      ...
      (result) => {
        ...
        return `Channel opened: ${shortId(result.funding_id)}`;
      },
    );
  }
  ...
  return (
    <main className="mx-auto w-[min(1180px,calc(100%_-_32px))] py-7 text-slate-900 max-md:w-[min(680px,calc(100%_-_24px))] max-md:py-5">
      ...
      <section
        className="mt-5 grid grid-cols-3 gap-3 max-md:grid-cols-1"
                    ...
                    <CardContent className="grid gap-4 p-4">
                      ...
                      <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-1">
                        ...
                        <Button
                          ...
                          type="button"
                          variant="secondary"
                          disabled={isLoading || pending?.role !== "responder"}
